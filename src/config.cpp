...
void Config::clear() {
    for (int i = 0; i < eepromLength; ++i) EEPROM.write(i, 0x0);
...
void Config::loadConfig() {
    ...
    if ((EEPROM.read(0) == CONFIG_VERSION[0]) &&
        ...
        for (int t = 0; t < sizeof(config);
             t++) *((char *)&config + t) = EEPROM.read(CONFIG_START + t);
    } else {
...
void Config::saveConfig() {
    ...
    for (int j = 0; j < sizeof(config);
         j++) EEPROM.write(CONFIG_START + j, *((char *)&config + j));
